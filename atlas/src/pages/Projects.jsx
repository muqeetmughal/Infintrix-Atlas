        ...
        <>
            ...
            <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {projects.map(p => (
                        <Card key={p.name} className="hover:shadow-2xl hover:-translate-y-1 transition-all duration-300 cursor-pointer group">
                            <div className="flex justify-between items-start mb-4">
                                <Badge className={PROJECT_STATUS_COLORS[p.status]}>{p.status}</Badge>
                                <button className="text-slate-300 hover:text-indigo-600 hover:bg-indigo-50 p-2 rounded-lg transition-all duration-200 opacity-0 group-hover:opacity-100"><MoreVertical size={20} /></button>
                            </div>
                            <Link to={`/tasks/${p.name}/kanban`}>
                            <h3 className="text-xl font-bold text-slate-900 mb-1 group-hover:text-indigo-600 transition-colors duration-200">{p.project_name}</h3>
                            </Link>
                            <p className="text-xs text-slate-400 mb-6 group-hover:text-slate-600 transition-colors duration-200">{p.name} • {p.project_type}</p>

                            <div className="space-y-2 mb-6">
                                <div className="flex justify-between text-[10px] font-black text-slate-400 uppercase tracking-widest">
                                    <span>Completion</span>
                                    <span className="text-indigo-600 font-black">{p.percent_complete}%</span>
                                </div>
                                <div className="w-full h-2 bg-slate-100 rounded-full overflow-hidden">
                                    <div className="h-full bg-indigo-500 transition-all duration-1000 rounded-full shadow-lg shadow-indigo-300" style={{ width: `${p.percent_complete}%` }} />
                                </div>
                            </div>
