...
const Header = () => {
    ...
    console.log(notifications_logs_query.data)

    const auth = useFrappeAuth();

    const markAsRead = async (notificationName) => {
        try {
            await fetch(`/api/method/frappe.desk.doctype.notification_log.notification_log.mark_as_read`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
                body: JSON.stringify({
                    docname: notificationName
                })
            });
                                    ...
                                    {notifications.length > 0 ? (
                                        notifications.map((notification) => (
                                            <div
                                                ...
                                                onClick={() => notification.read === 0 && markAsRead(notification.name)}
                                                className={`p-4 border-b border-slate-100 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700/50 cursor-pointer transition-colors ${notification.read === 0 ? 'bg-blue-50 dark:bg-blue-900/20' : ''
                                                    }`}
                                            >
                                                <div className="flex items-start space-x-3">
                                                    ...
                                                    <div className="flex-1">
                                                        <p className={`text-sm ${notification.read === 0 ? 'font-semibold' : 'font-medium'} text-slate-900 dark:text-slate-100`}
